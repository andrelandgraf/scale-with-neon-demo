...
async function cleanupFeature(): Promise<void> {
  ...
  if (!branchName) {
    ...
    try {
      ...
      if (currentBranch && currentBranch !== 'main' && currentBranch !== 'master' && currentBranch !== 'development') {
        branchName = currentBranch;
        console.log(`🔍 Using current git branch for cleanup: ${branchName}`);
      } else {
        console.error("❌ Please provide a feature branch name or switch to the feature branch:");
        console.error("   Usage: bun run cleanup-feature [branch-name]");
        console.error("   Example: bun run cleanup-feature andrelandgraf/feature-name");
        console.error("   Or checkout the feature branch first and run without arguments");
        process.exit(1);
      }
    } catch (error) {
  ...
  try {
    ...
    const featureBranch = branchesData.branches.find(branch => 
      branch.name === sanitizedBranchName || branch.name === branchName
    );

    if (!featureBranch) {
      console.warn(`⚠️  Database branch not found for '${branchName}' (also tried '${sanitizedBranchName}')`);
      console.log("Available branches:");
      branchesData.branches.forEach(branch => {
        console.log(`   • ${branch.name} (${branch.id})`);
      });
    } else {
      console.log(`✅ Found feature database branch: ${featureBranch.name} (${featureBranch.id})`);
      
      // Step 2: Delete the feature branch
      console.log("🗑️  Deleting feature database branch...");
      const deleteResponse = await fetch(`https://console.neon.tech/api/v2/projects/${projectId}/branches/${featureBranch.id}`, {
        method: 'DELETE',
        headers: {
          'Accept': 'application/json',
          'Authorization': `Bearer ${neonApiKey}`
      ...
      if (!deleteResponse.ok) {
        ...
        throw new Error(`Failed to delete branch: ${deleteResponse.status} ${errorText}`);
      }

      console.log(`✅ Database branch '${featureBranch.name}' deleted successfully`);
    }

    // Step 3: Find development branch for connection string
    console.log("🔍 Finding development database branch...");
    const developmentBranch = branchesData.branches.find(branch => 
      branch.name === 'development' || branch.name === 'dev' || branch.name === 'develop'
    );

    if (!developmentBranch) {
      console.warn("⚠️  Development branch not found. Looking for branches named 'development', 'dev', or 'develop'");
      console.log("Available branches:");
      branchesData.branches.forEach(branch => {
        console.log(`   • ${branch.name} (${branch.id})`);
      });
    ...
    console.log(`✅ Found development branch: ${developmentBranch.name} (${developmentBranch.id})`);

    // Step 4: Attempt to get connection details for development branch
    console.log("🔗 Attempting to update with development branch connection...");
    ...
    try {
      ...
      const endpointsResponse = await fetch(`https://console.neon.tech/api/v2/projects/${projectId}/branches/${developmentBranch.id}/endpoints`, {
        headers: {
          'Accept': 'application/json',
          'Authorization': `Bearer ${neonApiKey}`
        }
      ...
      if (endpointsResponse.ok) {
        ...
        if (endpoint) {
          ...
          console.log("💡 Please update your .env DATABASE_URL with the development branch connection:");
          console.log(`   Host: ${endpoint.host}`);
          console.log("   Get the full connection string from Neon Console → Development Branch → Connect");
          shouldUpdateEnv = false;
        }
      }
    } catch (error) {
      console.warn("⚠️  Could not get development branch connection details automatically");
    }

    if (!shouldUpdateEnv) {
      console.log("⚠️  Skipping automatic .env update - please manually update DATABASE_URL");
      ...
      console.log("   4. Update your .env DATABASE_URL with the development connection");
    }

    // Note: .env update is skipped - manual update required

    // Step 5: Optional git cleanup
    const shouldDeleteGitBranch = process.argv.includes('--delete-git-branch') || process.argv.includes('-g');
